...
- **「這一季結餘多少」問的是季末手上有多少**(`Finance.seasonHistory()` 的 `endTotal`
  = 季末公款餘額 + 當時的羽球庫存價值),不是這一季的收支相抵 ——
  公款餘額含前面幾季留下來的錢,一季一季比才看得出「有沒有正成長」。
  這一季自己賺賠多少是另一個數字(`balance`,收入 − 支出),兩個都要顯示,不要混為一談。
  季末餘額照**帳目的實際日期**累計(現金部位快照,提早繳的季費那天就進口袋了),
  季末庫存用 `Shuttles.stockValue(季末日期)`(帶日期才不會拿現在的庫存去充當歷史數字)。
  **季別結束後這個數字就凍結**(算到 `end` 那天為止),進行中的季別算到今天、標「進行中」,
  **還沒開始的季別不列出來**(`status === 'future'` 直接濾掉)——
  不擋的話下一季會拿今天的餘額當「季末結餘」,跟進行中的這一季顯示成一模一樣的數字。
- **這一季自己的收入 / 支出**(「各季結餘」卡、餘額卡的本季、季別的「查看季結算」)一律走 `Finance.seasonRows(season)`:
  **季費算進它繳的是哪一季**(`payment.seasonId`),不是繳錢那天落在哪一季 ——
  下一季的季費常常提早收,用日期切會把收入算到上一季,看起來像上一季特別賺、這一季特別差。
  其他帳(場次、買球、聚餐)照日期落在季別區間內算,期初餘額一律排除。
  這些數字**不另外存一份「季結算紀錄」**,全部從流水帳即時算 ——
  補記或修正舊場次時那一季的結餘會自動更新,不會有兩份對不起來的數字。
